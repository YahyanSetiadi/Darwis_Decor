...
<div class="flex items-center justify-between gap-4 mb-6">
    <div class="min-w-0">
        ...
        <h1 class="text-2xl font-semibold text-gray-800 leading-tight mt-1">Daftar Booking (Semua Status)</h1>
        <p class="text-sm text-gray-600 mt-1">Melihat seluruh booking agar admin bisa proses dan konfirmasi.</p>
    </div>
    ...
    <div class="flex items-center gap-3 flex-shrink-0">
        ...
        <a href="{{ route('admin.bookings.all') }}" class="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition">
            Refresh
        </a>
    </div>
</div>
...
<div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
    <div class="p-4 border-b border-gray-200 bg-gray-50">
        <div class="flex items-center justify-between gap-4">
            <span class="text-sm font-medium text-gray-700">Daftar seluruh booking</span>
        </div>
    </div>
                        ...
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            @if($b->status === 'pending')
                                <form method="POST" action="{{ route('admin.bookings.approve', $b->id) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="px-4 py-2 rounded-xl bg-green-600 text-white text-xs font-semibold hover:bg-green-700 transition">Approve</button>
                                </form>
                            @else
                                <span class="text-xs text-gray-400">Sudah diproses</span>
                            @endif
                            <a href="{{ route('user.bookings.invoice', $b->id) }}" class="ml-3 px-3 py-2 rounded-xl bg-stone-100 text-gray-700 text-xs font-semibold hover:bg-stone-200 transition">Lihat Invoice</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-12 text-center text-gray-500">Belum ada booking.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
